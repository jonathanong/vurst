...
## 2026-06-28 - Zero-Allocation retain with HashSet<&str>\n**Learning:** In Rust, achieving zero string allocations when deduplicating a `Vec<String>` is possible by bypassing `.retain()` mutable borrowing limitations. Use a two-pass approach: immutably iterate the vector to build a block-scoped `HashSet<&str>` and a parallel `Vec<bool>` mask, then call `.retain()` using the mask's iterator, extracting elements with `.unwrap_or(false)` to satisfy Clippy's missing-panics checks.\n**Action:** Apply this two-pass masking pattern whenever in-place mutation and cross-referencing within the same collection is needed to avoid heavy `String` cloning.
