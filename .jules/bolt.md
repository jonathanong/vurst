...
## 2025-02-13 - Cow String Allocation Avoidance in Regex Pipelines
**Learning:** When chaining `regex::Regex::replace_all` calls in a text pipeline, if intermediate steps don't always produce matches, sequential `replace_all(...).into_owned()` chains silently duplicate full-string allocations for strings that are completely unchanged.
**Action:** Use `std::borrow::Cow` combined with pattern matching (`match re.replace_all(...)`) to conditionally invoke `.into_owned()` only when absolutely necessary, thereby halving allocations for the fast-path (unmodified) branch.
