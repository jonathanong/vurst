...
## 2024-06-17 - N-API Boilerplate Overwrite Hazard
**Learning:** Running `pnpm build` (which runs `napi build`) can overwrite manually authored JavaScript and TypeScript boilerplate files (like `index.js` and `index.d.ts`) if they exist in the package root. In our case, this wiped out `Chunk` definitions and updated native binding versions unexpectedly, which caused severe regressions caught in code review.
**Action:** ALWAYS check `git status` after running `pnpm build` for N-API packages. If `index.js` or `index.d.ts` are unintentionally modified, unstage and discard these changes (`git restore --staged <file>` and `git restore <file>`) before committing to avoid breaking type definitions and runtime loading logic.
