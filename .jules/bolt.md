...
**Action:** When designing N-API wrapper options that will be passed into `'static'` lifetimes (such as Ammonia's `attribute_filter`), use `Arc<str>` instead of `String` and `Arc<[T]>` instead of `Vec<T>` to avoid deep cloning overhead, especially when the options are reused across many calls.
...
**Learning:** In Rust, chained `str::replace` after `regex::replace_all` should not eagerly call `.into_owned()`. `str::replace` always allocates a new `String` even if the pattern isn't found.
