...
fn detect_ai_generated_text_rejects_out_of_bounds_threshold() {
    let error_negative =
        detect_ai_generated_text("some text", -0.1).expect_err("should reject negative threshold");
    assert_eq!(
        error_negative,
    ...
    let error_too_large =
        detect_ai_generated_text("some text", 1.1).expect_err("should reject threshold > 1.0");
    assert_eq!(
        error_too_large,
